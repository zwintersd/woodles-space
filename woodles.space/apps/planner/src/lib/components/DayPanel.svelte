...
<svelte:window
	onkeydown={(e) => {
		if (e.key === 'Escape' && store.activeDayKey != null && store.editingTaskId == null && !store.composing) close();
	}}
/>

{#if store.activeDayKey != null && store.editingTaskId == null && !store.composing}
