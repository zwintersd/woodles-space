...
First Water resident atlas:

- `creatures/deepwater_fish_swim_sheet.png` — a 4 × 3, 12-frame watercolor
  swim atlas generated by the local Sprite Studio. Once the soft-bodied swimmers
  have been observed, it makes an occasional deep-water pass. Its matching JSON
  sits beside it for provenance; `WorldCanvas.svelte` selects the correct cell at
  12 FPS and uses frame zero when reduced motion is requested.
