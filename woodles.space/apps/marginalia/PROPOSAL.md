...
category pack — typed creature / biome / lore records absorbed from the retired
`marginalia-devlog` app — used to be a possible content pipeline for this; spores
itself has since retired into write (CONVERGENCE.md §8), and the worldbuilding pack
had no home among the four apps that stayed, so this idea needs a different source
