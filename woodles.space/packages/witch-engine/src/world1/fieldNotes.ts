// Field notes — a live, per-event observation log. Short lines, drafted in
// Brianna's register but templated so they don't need one hand-written entry
// per life; edit freely, nothing here gates progress.
//
// This is `world1Def.fieldNotes`'s data, in the shape @woodles/witch-engine's
// `FieldNotesDef` expects. `pickLine`/`fillTemplate` — the generic,
// content-free half of what produces a line from this data — live in
// text.ts; `World`'s tick calls them directly against this data.
//
// World 1's actual content — bundled here the same way tuning.ts is; see
// that file's header note.

import type { LifeCategory, LifeDomain } from '../types.js';
