// Idle tuning — every balance number for World 1 in one place, so the loop
// stays legible and min/max-able by inspection.
//
// This is Marginalia's actual, shipped World 1 — bundled here the same way
// @woodles/incremental-core bundles its own example fixtures, so an app
// besides apps/marginalia (a balance tool, a tuning instrument) has
// something real to point at rather than an abstract schema. See
// world1/def.ts, which assembles these into a MarginaliaDef.
